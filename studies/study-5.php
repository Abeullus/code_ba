...
/* Hier steht der Code und Content für den 5. Versuchsdurchlauf der Studie.  Der Teilnehmer die Aufgabe, die vorgegebenen Wörter im Menü zu wählen, um sie so in den Reiter "meine Liste" hinzuzufügen.  */ 
    ...
    /* Hier werden die Daten des 4. Versuchsdurchlaufs in die Datenbank geschrieben. Dies erfolgt zeitlich versetzt, da so nicht während eines Versuchsdurchlaufs das Fenster neu geladen werden kann um bessere Ergebnisse zu erzielen. */
    mysqli_query($mysql, 'INSERT INTO `Experiment` (`Experiment_ID`, `Durchgang_ID`, `Menu_ID`, `User_ID`, `UserSuccessRate`, `TimeOnTask`, `TaskErrorRate`, `ClicksTotal`, `KLM`, `KLM-Time`, `Clicks`, `SystemInfo`) VALUES (' . $_SESSION['exp'] . ', 4, ' . $_SESSION['study'] . ', ' . session_id() . ', ' . (float)$_POST['tsr'] . ', ' . (float)$_POST['realtime'] . ', ' . (int)$_POST['errors'] . ', ' . (int)$timings['bb'] . ', \'' . $_POST['timings'] . '\', ' . (float)$_POST['time'] . ', \'' . $_POST['clicks'] . '\', \'' . $_POST['platform'] . '\')');
    ...
    $wordList = mysqli_query($mysql, 'SELECT `WordList_ID`, `WordsToSearch` FROM `WordList` WHERE `Functions_ID`=' . $study[1] . ' ORDER BY `WordList_ID` ASC LIMIT 4, 1')->fetch_row();
